...
#      ansible.playbook = "deploy_pythonda.yml"
##     ansible.groups = {
...
# end
  config.vm.define "pythonda" do |pythonda|
    pythonda.vm.box = "clouddood/RH7.5_baserepo"
    pythonda.vm.hostname = "pythonda"
    pythonda.vm.network "private_network", ip: "192.168.60.159"
#   pythonda.vm.network "private_network", ip: "192.168.60.159", nic_type: "virtio"
    pythonda.vm.provision "shell", :inline => "sudo echo '192.168.60.159 pythonda.local pythonda' >> /etc/hosts"

##  Use Main / Update in Vagrant provision command ### $vagrant provision --provision-with shell/main/update

    # Default
    pythonda.vm.provision "main", type: "ansible" do |ansible|
      ansible.playbook = "deploy_pythonda_DEV.local.yml"
#     ansible.playbook = "deploy_pythonda_DEV.local.yml"
#     ansible.playbook = "deploy_pythondabTest.yml"
      ansible.inventory_path = "vagrant_hosts"
    ...
    # Update
#   pythonda.vm.provision "update", type: "ansible" do |ansible|
#     ansible.playbook = "deploy_pythondaPatchRH7_DEV.local.yml"
#     ansible.playbook = "deploy_pythondaTestRH7.yml"
#     ansible.inventory_path = "vagrant_hosts"
#     #ansible.tags = ansible_tags
#     #ansible.verbose = ansible_verbosity
#     #ansible.extra_vars = ansible_extra_vars
